(ns ^:figwheel-hooks tailwind.examples
  (:require
    [cljsjs.emotion]
...
(defn card-2 []
  [:div {:class (tw! "max-w-sm w-full lg:max-w-full lg:flex")}
   [:div
    {:css   [{:background-image "url('https://tailwindcss.com/img/card-left.jpg')"
              :title            "Woman holding a mug"}]
     :class (tw! "h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-t"
                 "lg:rounded-t-none lg:rounded-l text-center overflow-hidden")}]
   [:div {:class (tw! "border-r border-b border-l border-gray-400 lg:border-l-0"
    ...
    [:div {:class (tw! "mb-8")}
     [:p {:class (tw! "text-sm text-gray-600 flex items-center")}
      [:svg
       {:class (tw! "fill-current text-gray-500 w-3 h-3 mr-2")
    ...
    [:div {:class (tw! "flex items-center")}
     [:img {:class (tw! "w-10 h-10 rounded-full mr-4")
            :src   "https://tailwindcss.com/img/jonathan.jpg"
            :alt   "Avatar of Jonathan Reinink"}]
     [:div {:class (tw! "text-sm")}
      [:p {:class (tw! "text-gray-900 leading-none")} "Jonathan Reinink"]
      [:p {:class (tw! "text-gray-600")} "Aug 18"]]]]])
...
(defn form-1 []
  (let [d1 (tw! "md:flex md:items-center mb-6")
    ...
    [:form {:class (tw! "w-full max-w-sm")}
     [:div {:class d1}
      [:div {:class (tw! "md:w-1/3")}
       [:label {:for "inline-full-name" :class l1} "Full Name"]]
      [:div {:class (tw! "md:w-2/3")}
       [:input#inline-full-name
        {:type "text" :placeholder "Jane Doe" :class i1}]]]
     [:div {:class d1}
      [:div {:class (tw! "md:w-1/3")}
       [:label {:for "inline-username" :class l1} "Password"]]
      [:div {:class (tw! "md:w-2/3")}
       [:input#inline-username
        {:class i1 :type "password" :placeholder "******************"}]]]
     [:div {:class d1}
      [:div {:class (tw! "md:w-1/3")}]
      [:label {:class (tw! "md:w-2/3 block text-gray-500 font-bold")}
       [:input {:type "checkbox" :class (tw! "mr-2 leading-tight")}]
       [:span {:class (tw! "text-sm")} "Send me your newsletter!"]]]
     [:div {:class (tw! "md:flex md:items-center")}
      [:div {:class (tw! "md:w-1/3")}]
      [:div {:class (tw! "md:w-2/3")}
       [:button {:type "button" :class b1} "Sign Up"]]]]))
...
(defn form-2 []
  (let [i1 (tw! "appearance-none bg-transparent border-none w-full"
    ...
    [:form {:class (tw! "w-full max-w-sm")}
     [:div {:class (tw! "flex items-center border-b border-teal-500 py-2")}
      [:input {:class i1 :type "text" :placeholder "Jane Doe" :aria-label "Full name"}]
      [:button {:class b1 :type "button"} "Sign Up"]
      [:button {:class b2 :type "button"} "Cancel"]]]))

; https://tailwindcss.com/components/navigation/#responsive-header
(defn header []
  [:nav {:class (tw! "flex items-center justify-between flex-wrap bg-teal-500 p-6")}
   [:div {:class (tw! "flex items-center flex-shrink-0 text-white mr-6")}
    [:svg {:class   (tw! "fill-current h-8 w-8 mr-2")
           :width   "54"
           :height  "54"
           :viewBox "0 0 54 54"
           :xmlns   "http://www.w3.org/2000/svg"}
     [:path {:d "M13.5 22.1c1.8-7.2 6.3-10.8 13.5-10.8 10.8 0 12.15 8.1 17.55 9.45 3.6.9 6.75-.45 9.45-4.05-1.8 7.2-6.3 10.8-13.5 10.8-10.8 0-12.15-8.1-17.55-9.45-3.6-.9-6.75.45-9.45 4.05zM0 38.3c1.8-7.2 6.3-10.8 13.5-10.8 10.8 0 12.15 8.1 17.55 9.45 3.6.9 6.75-.45 9.45-4.05-1.8 7.2-6.3 10.8-13.5 10.8-10.8 0-12.15-8.1-17.55-9.45-3.6-.9-6.75.45-9.45 4.05z"}]]
    [:span {:class (tw! "font-semibold text-xl tracking-tight")} "Tailwind CSS"]]
   ...
   [:div {:class (tw! "w-full block flex-grow lg:flex lg:items-center lg:w-auto")}
    (let [a (tw! "block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4")]
      [:div {:class (tw! "text-sm lg:flex-grow")}
       [:a {:href "#responsive-header" :class a} "Docs"]
       [:a {:href "#responsive-header" :class a} "Examples"]
       [:a {:href "#responsive-header" :class a} "Blog"]])
    [:div
     [:a {:href  "#"
...
(defn spacer []
  [:div {:class (tw! "my-4 w-full border-b border-gray-400")}])
